...
void load_bank_log() {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%s %d", bank_id, &balance) == 2) {
            // Add the account to our list
            strncpy(accounts[account_count].bank_id, bank_id, sizeof(accounts[account_count].bank_id));
            accounts[account_count].balance = balance;
            accounts[account_count].is_active = 1;
...
void update_bank_log() {
    FILE *fp = fopen(log_file, "w");
    if (fp == NULL) {
        perror("fopen log_file");  // Print the specific error with the filename
        printf("Failed to open log file: %s\n", log_file);  // Print the actual path
        return;
    }
    
    // Add error checking for write operations
    int write_success = 1;
    ...
    if (fprintf(fp, "# %s Log file updated @%02d:%02d April %d %d\n", 
            bank_name, t->tm_hour, t->tm_min, 
            t->tm_mday, 1900 + t->tm_year) < 0) {
        write_success = 0;
    }
    ...
    for (int i = 0; i < account_count; i++) {
        if (accounts[i].is_active) {
            if (fprintf(fp, "%s %d\n", accounts[i].bank_id, accounts[i].balance) < 0) {
                write_success = 0;
                break;
            }
        }
    ...
    if (write_success) {
        printf("Bank log successfully updated at %s\n", log_file);
    } else {
        printf("Error writing to bank log file\n");
    }
}
